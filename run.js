...
var content_element;
var content_scroll_ratio;

<<<<<<< HEAD
chrome.runtime.sendMessage({type: "getContentElement", message: location.href}, function (response) {
    if (response) {
...
>>>>>>> parent of 421c7d4... Merge pull request #4 from ThisIsNoZaku/master
    }
});

<<<<<<< HEAD
$(document).ready(function () {
    $(document).scroll(updateContentScrollMetrics)
    chrome.runtime.sendMessage({type: "getUser"}, function (response) {
        ...
        if (data.url) {
            console.log("Sending to background to write");
...
//Get the source content element of the current page.

//Update the maximum scrolled portion of the content element.
    function updateContentScrollMetrics() {
        if(!content_element){
            content_element = $("body");
        }
        var content_height = content_element.height();
        //Offset is the distance between the top of the content element and the viewport.
        var calculated_scroll_ratio = 100 - (Math.max(content_height - content_element.offset().top, 0) / content_height);
        //Only update if more of the content has been displayed.
        content_scroll_ratio = Math.max(calculated_scroll_ratio, content_scroll_ratio);
