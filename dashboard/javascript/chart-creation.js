var color = ["#393B79", "#3182BD", "#E6550D", "#31A354", "#CE6BDB", "#17BECF", "#ED6A5A", "#EFCB68", "#88D18A", "#3E5C76", "#D1D1D1", "#114B5F", "#FFE74C", "#FE5F55", "#35A7FF", "#468966", "#B64926", "#1695A3", "#BDD4DE", "#832F5C", "#382513", "#363942", "#405952", "#F54F29", "#083643", "#CEF09D", "#FF974F", "#91BED4", "#365FB7", "#D23600", "#FC9D9A", "#83AF9B", "#791F33", "#78C0F9", "#FFDBE6", "#B9121B"];
...
function createBarChart(dataset) {
    ...
    var svg = d3.select("div.bar-chart").append("svg").attr("class", "chart chart--font").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    ...
    var yAxis = d3.svg.axis().scale(y).orient("left").ticks(5).tickSize(-width, 0, 0).tickFormat(function(d) {
        return d;
    });
    var xAxis = d3.svg.axis().scale(x)
    svg.append("g").attr("class", "y-axis").call(yAxis)
    svg.append("g").attr("class", "x-axis").attr("transform", "translate(0," + height + ")").call(xAxis);
    var groups = svg.selectAll("g.time").data(dataset).enter().append("g").attr("class", "time").attr("fill", function(d, i) {
        return color[i];
    });
    var rect = groups.selectAll("rect").data(function(d) {
        ...
        return x(d.x);
    }).attr("class", "bar-chart-bar").attr("y", height).attr("width", x.rangeBand()).attr("height", 0);
