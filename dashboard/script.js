    ...
    chrome.runtime.sendMessage({type: "getUser"}, function (user) {
        chrome.extension.getBackgroundPage()._firebase.then(function (firebase) {
            firebase.database().ref("users/" + user.id).once("value").then(function (userSnapshot) {
                ...
                for (key in articles) {
                    article_definitions[key] = firebase.database().ref("articles/" + key).once("value");
                }
                Promise.resolve(article_definitions).then(function (articleSnapshots) {
                    var daysBack = 10;
                    ...
                    createTable(articlesRead, articleSnapshots);
                    ...
                    function createTable(userArticleInformation, articleInformation) {
                        ...
                        for (var key in userArticleInformation) {
                            if (userArticleInformation.hasOwnProperty(key)) {
                                ...
                                var currentArticle = articleInformation[key];
                                ...
                                userArticleInformation.push(articleData);
                            }
                        }

                        userArticleInformation.sort(function (a, b) {
                            return Date.parse(b[1]) - Date.parse(a[1]);
                        });

                        for (var i = 0; i < userArticleInformation.length; i++) {
                            var tr = document.createElement("TR");
                            for (var j = 0; j < userArticleInformation[j].length; j++) {
                                var td = document.createElement("TD");
                                td.appendChild(document.createTextNode(userArticleInformation[i][j]));
                                tr.appendChild(td)
                            }
