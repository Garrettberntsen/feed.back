...
$(document).ready(function () {
    bg.database.then(function (database) {
        function buildPieData(user, current_article) {
            articles = user && user.articles;

            $('#title').text(current_article ? current_article.title : "");
            if (current_article) {
                if (current_article.author) {
                    var authors = '';
                    for (var author in current_article.author) {
                        authors += current_article.author[author] + ', ';
                    }
                    authors = authors.substring(0, authors.length - 2);
                    $('#author').text('by ' + authors);
                }
                $('#read-count').text(Object.keys(current_article.readers).length);
            }

            $('#leanRating').barrating({
                theme: 'bars-movie',
                initialRating: current_article ? current_article.lean : 0,
                onSelect: function (value, text) {
                    setLeanColor(value);
                    database.ref('users/' + bg.user_id + '/articles/' + article_key + '/lean').set($('#leanRating').val());
                }
            });
            if (current_article.lean) {
                setLeanColor(current_article.lean);
            }
            $('#starRating').barrating({
                theme: 'fontawesome-stars',
                initialRating: current_article.stars,
                onSelect: function (value, text) {
                    database.ref('users/' + bg.user_id + '/articles/' + article_key + '/stars').set($('#starRating').val());
                    $('#avg-rating-message').show();
                }
            });
            if (current_article.stars) {
                $('#avg-rating-message').show();
            }
            $("form").show();

            for (var current_article in articles) {
                source = articles[current_article].source;
                if (source !== undefined) {
